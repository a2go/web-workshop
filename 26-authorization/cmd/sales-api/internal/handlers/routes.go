...
func API(db *sqlx.DB, errorLog, infoLog *log.Logger, authenticator *auth.Authenticator) http.Handler {
...

	{
		...
		app.Handle(http.MethodPost, "/v1/products", p.Create, authmw.Authenticate, authmw.HasRole(auth.RoleAdmin))
		...
		app.Handle(http.MethodPut, "/v1/products/{id}", p.Update, authmw.Authenticate, authmw.HasRole(auth.RoleAdmin))
